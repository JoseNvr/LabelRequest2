...
import { PedidosFinalizadosService } from "src/app/modules/pedidosfinalizados/pesfinalizados.service";
...
export class PedidosFinalizadosComponent implements OnInit, OnDestroy {
  ...
  constructor(private pedidosfinalizadosService: PedidosFinalizadosService,private notify: Notify) {
    this.subscriptions = [];
  }
  ...
  ngOnInit() {
    ...
    const getTestSub: Subscription = this.pedidosfinalizadosService
    ...
    const postTestSub: Subscription = this.pedidosfinalizadosService
  ...
  mProcessUnitSerialNumber(data: any) {
    this.pedidosfinalizadosService.ProcessUnitSerialNumber(data).subscribe(Result => {
      console.log(Result);
      if (!Result.success) {
  ...
  mValidatePasswordBarcode(data: any) {
    this.pedidosfinalizadosService.ValidatePasswordBarcode(data).subscribe(Result => {
      console.log(Result);
      document.getElementById('txtPasswordBarcode').removeAttribute('disabled');
      $('#txtPasswordBarcode').val("");
      $('#txtPasswordBarcode').focus();
