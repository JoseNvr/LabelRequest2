...
export class ReimpresionComponent implements OnInit, OnDestroy {
  ...
  constructor(private reimpresionService: PrincipalService,private notify: Notify) {
    this.subscriptions = [];
  }
  ngOnInit() {
    //Cargar interfaz
    ...
    const getTestSub: Subscription = this.reimpresionService
    ...
    const postTestSub: Subscription = this.reimpresionService
  ...
  mProcessUnitSerialNumber(data: any) {
    this.reimpresionService.ProcessUnitSerialNumber(data).subscribe(Result => {
      console.log(Result);
      if (!Result.success) {
  ...
  mValidatePasswordBarcode(data: any) {
    this.reimpresionService.ValidatePasswordBarcode(data).subscribe(Result => {
      console.log(Result);
      document.getElementById('txtPasswordBarcode').removeAttribute('disabled');
      $('#txtPasswordBarcode').val("");
      $('#txtPasswordBarcode').focus();
