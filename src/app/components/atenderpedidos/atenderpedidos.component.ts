...
import { AtenderPedidosService } from "src/app/modules/atenderpedidos/atenderpedidos.service";
import { Subscription } from "rxjs";
import { Constants } from "../../helpers/constats";
import { Notify } from "../../modules/notify/notify";
import { constants } from "buffer";
@Component({
  selector: "app-atenderpedidos",
  templateUrl: "./atenderpedidos.component.html",
  styleUrls: ["./atenderpedidos.component.scss"]
})
export class AtenderPedidosComponent implements OnInit, OnDestroy {
  ...
  constructor(private atenderpedidosService: AtenderPedidosService,private notify: Notify) {
    this.subscriptions = [];
  }
  ngOnInit() {
    //Cargar interfaz
    ...
    const getTestSub: Subscription = this.atenderpedidosService
    ...
    const postTestSub: Subscription = this.atenderpedidosService
  ...
  mProcessUnitSerialNumber(data: any) {
    this.atenderpedidosService.ProcessUnitSerialNumber(data).subscribe(Result => {
      console.log(Result);
      if (!Result.success) {
  ...
  mValidatePasswordBarcode(data: any) {
    this.atenderpedidosService.ValidatePasswordBarcode(data).subscribe(Result => {
      console.log(Result);
      document.getElementById('txtPasswordBarcode').removeAttribute('disabled');
      $('#txtPasswordBarcode').val("");
      $('#txtPasswordBarcode').focus();
