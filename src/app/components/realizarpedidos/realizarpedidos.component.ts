...
import { RealizarPedidosService } from "src/app/modules/realizarpedidos/realizarpedidos.service";
import { Subscription } from "rxjs";
import { Constants } from "../../helpers/constats";
import { Notify } from "../../modules/notify/notify";
import { constants } from "buffer";
@Component({
  selector: "app-realizarpedidos",
  templateUrl: "./realizarpedidos.component.html",
  styleUrls: ["./realizarpedidos.component.scss"]
})
export class RealizarPedidosComponent implements OnInit, OnDestroy {
  ...
  constructor(private realizarpedidosService: RealizarPedidosService,private notify: Notify) {
    this.subscriptions = [];
  }
  ngOnInit() {
    //Cargar interfaz
    ...
    const getTestSub: Subscription = this.realizarpedidosService 
    ...
    const postTestSub: Subscription = this.realizarpedidosService
  ...
  mProcessUnitSerialNumber(data: any) {
    this.realizarpedidosService.ProcessUnitSerialNumber(data).subscribe(Result => {
      console.log(Result);
      if (!Result.success) {
  ...
  mValidatePasswordBarcode(data: any) {
    this.realizarpedidosService.ValidatePasswordBarcode(data).subscribe(Result => {
      console.log(Result);
      document.getElementById('txtPasswordBarcode').removeAttribute('disabled');
      $('#txtPasswordBarcode').val("");
      $('#txtPasswordBarcode').focus();
