...
import { AdministradorService } from "src/app/modules/administrador/administrador.service";
...
export class AdministradorComponent implements OnInit, OnDestroy {
  ...
  constructor(private administradorService: AdministradorService,private notify: Notify) {
    this.subscriptions = [];
  }
  ...
  ngOnInit() {
    ...
    const getTestSub: Subscription = this.administradorService
    ...
    const postTestSub: Subscription = this.administradorService
  ...
  mProcessUnitSerialNumber(data: any) {
    this.administradorService.ProcessUnitSerialNumber(data).subscribe(Result => {
      console.log(Result);
      if (!Result.success) {
  ...
  mValidatePasswordBarcode(data: any) {
    this.administradorService.ValidatePasswordBarcode(data).subscribe(Result => {
      console.log(Result);
      document.getElementById('txtPasswordBarcode').removeAttribute('disabled');
      $('#txtPasswordBarcode').val("");
      $('#txtPasswordBarcode').focus();
