...
import { VerPedidosService } from "src/app/modules/verpedidos/verpedidos.service";
...
@Component({
  ...
  styleUrls: ["./verpedidos.component.scss"]
})
export class VerPedidosComponent implements OnInit, OnDestroy {
  ...
  constructor(private verpedidosService: VerPedidosService,private notify: Notify) {
    this.subscriptions = [];
  }
  ngOnInit() {
    //Cargar interfaz
    ...
    const getTestSub: Subscription = this.verpedidosService
    ...
    const postTestSub: Subscription = this.verpedidosService
  ...
  mProcessUnitSerialNumber(data: any) {
    this.verpedidosService.ProcessUnitSerialNumber(data).subscribe(Result => {
      console.log(Result);
      if (!Result.success) {
  ...
  mValidatePasswordBarcode(data: any) {
    this.verpedidosService.ValidatePasswordBarcode(data).subscribe(Result => {
      console.log(Result);
      document.getElementById('txtPasswordBarcode').removeAttribute('disabled');
      $('#txtPasswordBarcode').val("");
      $('#txtPasswordBarcode').focus();
